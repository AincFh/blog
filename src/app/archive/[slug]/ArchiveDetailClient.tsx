...
export default function ArchiveDetailClient({ slug }: ArchiveDetailClientProps) {
        ...
        <div className="max-w-4xl mx-auto py-8 px-4">
            ...
            {allTags.length > 0 && (
                <div className="mb-6 bg-white dark:bg-neutral-800 rounded-xl p-4 shadow-sm border border-neutral-200 dark:border-neutral-700">
                    <h3 className="text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-3">按标签过滤</h3>
                    <div className="flex flex-wrap gap-2">
                        <button
                            onClick={() => setSelectedTag(null)}
                            className={`px-3 py-1 rounded-full text-xs font-medium transition-all duration-200 ${selectedTag === null
                        ...
                        {allTags.map((tag) => (
                            <button
                                ...
                                className={`px-3 py-1 rounded-full text-xs font-medium transition-all duration-200 ${selectedTag === tag
                                    ? "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300"
                                    : "bg-neutral-100 text-neutral-700 dark:bg-neutral-700 dark:text-neutral-300 hover:bg-neutral-200 dark:hover:bg-neutral-600"
                                    }`}
                            >
            ...
            {filteredArticles.length > 0 ? (
                        ...
                        <article key={article.id} className="bg-white dark:bg-neutral-800 rounded-xl overflow-hidden shadow-sm border border-neutral-200 dark:border-neutral-700 hover:shadow-md transition-all duration-300">
                            <div className="p-6">
                                ...
                                <h2 className="text-xl font-semibold mb-2">
                                    <Link href={`/posts/${article.id}`} className="text-neutral-900 dark:text-neutral-100 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                                        {article.title}
                                    </Link>
                                </h2>
                ...
                <div className="bg-white dark:bg-neutral-800 rounded-xl p-8 text-center shadow-sm border border-neutral-200 dark:border-neutral-700">
                    ...
                    <p className="text-neutral-600 dark:text-neutral-400">
                        {selectedTag ? `没有找到标签为 "${selectedTag}" 的文章` : "该月份暂无文章"}
                    </p>
                </div>
            )}
        </div>
