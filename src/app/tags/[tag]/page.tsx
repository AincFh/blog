...
export default async function TagDetailPage({ params }: { params: Promise<{ tag: string }> }) {
  ...
  const articles = mockTagData[decodedTag as keyof typeof mockTagData] || [];

  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
      <div className="mb-8">
        <a href="/tags" className="inline-flex items-center text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 mb-4 transition-colors">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {article.tags.map((t, idx) => (
                      <a
                        key={`article-${article.id}-tag-${idx}`}
                        href={`/tags/${encodeURIComponent(t)}`}
                        className={`px-2 py-1 rounded text-xs hover:bg-neutral-200 dark:hover:bg-neutral-600 transition-colors ${t === decodedTag
